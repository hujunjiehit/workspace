...
function login(userName,passWord)
	touchClick(626,1222); --点击我的tab，拉起登陆界面
...
function startToXiadan()
	for index = 1,7 do	
		...
		m,n = findColorInRegionFuzzy(0x33c774,90,495,1097,709, 1170);
		mSleep(2000)  --#################################################### 优化
		nLog("m = "..m.." n = "..n)
		if m ~= -1 and n ~= -1 then
			touchClick(m+97,n+21);
			mSleep(1000)
			...
			if color == 0x666666 then
				...
				--可以选课
				touchClick(m+97,n+21); --点击稍后支付，然后循环等待，直到付款成功
				repeat
					-- body
					mSleep(1000);
					nLog("please whait...")
			...
			--跳转失败，代表没有更多团课
			if getColor(510, 1119) == 0xd8d8d8 then
				--已报名，按钮灰色
				os.execute("input keyevent 4");
				mSleep(1000)
			end
